...
import { tipoProgramaApi, type TipoProgramaView } from "../../api";

interface CatalogRow extends TipoProgramaView {}
...
export function TipoProgramaView() {
	...
	const loadRows = async () => {
		...
		try {
			const data = await tipoProgramaApi.listar();
			setRows(data);
		} catch (error) {
			console.error(error);
			setLoadError("No se pudo cargar el tipo de programa.");
		} finally {
			setIsLoading(false);
		}
	};
	...
	const handleAddRow = async () => {
		...
		try {
			const created = await tipoProgramaApi.crear({ nombre });
			...
			showToast(
				"Error",
				error instanceof Error ? error.message : "No se pudo crear el tipo de programa.",
				"error"
			);
		}
	};
	...
	const saveEditRow = async () => {
		...
		try {
			const updated = await tipoProgramaApi.actualizar({
				id: editingRowId,
				nombre,
			});
			...
			showToast(
				"Error",
				error instanceof Error ? error.message : "No se pudo actualizar el tipo de programa.",
				"error"
			);
		}
	};
	...
	const confirmRemoveRow = async () => {
		...
		try {
			await tipoProgramaApi.eliminar(pendingDeleteRow.id);
			...
			showToast(
				"Error",
				error instanceof Error ? error.message : "No se pudo eliminar el tipo de programa.",
				"error"
			);
		}
	};

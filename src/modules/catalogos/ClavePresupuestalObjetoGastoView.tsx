...
import { clavePresupuestalApi, type ClavePresupuestalView } from "../../api";

type CatalogStatus = "ACTIVO" | "INACTIVO";
interface CatalogRow extends ClavePresupuestalView {}
...
export function ClavePresupuestalObjetoGastoView() {
	...
	const loadRows = async () => {
		...
		setLoadError(null);
		try {
			const rows = await clavePresupuestalApi.listar();
			setRows(rows);
		} catch (error) {
			console.error(error);
			setLoadError("No se pudieron cargar las claves presupuestales.");
		} finally {
			setIsLoading(false);
		}
	};
	...
	const filteredAndSortedRows = useMemo(() => {
		...
		return [...filtered].sort(
			(a, b) =>
				String(a[sortConfig.key as keyof CatalogRow])
					.localeCompare(String(b[sortConfig.key as keyof CatalogRow]), "es", { numeric: true }) *
	...
	const handleAddRow = async () => {
		...
		try {
			const created = await clavePresupuestalApi.crear({ nombre });
			setRows((prev) => [created, ...prev]);
			closeInsertRow();
			showToast("Registro creado", `Registro ${created.id} creado correctamente.`);
		} catch (error) {
			console.error(error);
			showToast("Error", error instanceof Error ? error.message : "No se pudo crear la clave presupuestal.", "error");
		}
	};
	...
	const saveEditRow = async () => {
		...
		try {
			const updated = await clavePresupuestalApi.actualizar({ id: editingRowId, nombre });
			setRows((prev) => prev.map((row) => (row.id === updated.id ? updated : row)));
			cancelEditRow();
			showToast("Registro actualizado", `Registro ${updated.id} editado correctamente.`);
		} catch (error) {
			console.error(error);
			showToast("Error", error instanceof Error ? error.message : "No se pudo actualizar la clave presupuestal.", "error");
		}
	};
	...
	const confirmRemoveRow = async () => {
		...
		try {
			await clavePresupuestalApi.eliminar(pendingDeleteRow.id);
			...
			console.error(error);
			showToast("Error", "No se pudo eliminar la clave presupuestal.");
		}
	};
		...
		<div className="flex flex-col h-full min-h-0 bg-slate-50 p-2 lg:p-3 overflow-hidden">
			<div className="w-full min-h-0 flex-1 flex flex-col">
				<PageCard className="h-full min-h-0 flex-1 flex flex-col">
					<ViewHeader
						title="Clave presupuestal / objeto gasto"
						action={
							!showInsertRow ? (
								...
								insertRow={
									showInsertRow ? (
										<tr className="bg-blue-50/40 border-b border-blue-100">
											<td className="px-2 py-2 border-r border-slate-100/70 text-[11px] text-slate-500">AUTOGENERADO</td>
											<td className="px-2 py-2 border-r border-slate-100/70">
												<Input autoFocus value={draftNombre} onKeyDown={handleInsertRowKeyDown} onChange={(e) => setDraftNombre(e.target.value.toUpperCase())} placeholder="NOMBRE" />
											</td>
											<td className="px-2 py-2 border-r border-slate-100/70">
												<span className="inline-flex items-center px-2 py-0.5 rounded-full text-[10px] font-bold uppercase bg-emerald-100 text-emerald-700">ACTIVO</span>
											</td>
											<td className="px-2 py-2 text-center text-[11px] text-slate-500">ENTER = GUARDAR</td>
										</tr>
									) : null
								}
								renderRow={(row, index) => {
									...
									return (
										<tr key={row.id} className={`border-b border-slate-100/60 ${index % 2 === 0 ? "bg-white" : "bg-slate-50/30"}`}>
											<td className="px-3 py-2 border-r border-slate-100/70 font-medium text-slate-700">{row.id}</td>
											<td className="px-3 py-2 border-r border-slate-100/70 uppercase">
												{isEditing ? (
													<Input value={editingNombre} onChange={(e) => setEditingNombre(e.target.value.toUpperCase())} placeholder="NOMBRE" />
												) : (
													row.nombre
												)}
											</td>
											<td className="px-3 py-2 border-r border-slate-100/70">
												<span className="inline-flex items-center px-2 py-0.5 rounded-full text-[10px] font-bold uppercase bg-emerald-100 text-emerald-700">{row.estatus}</span>
											</td>

	...
	const filteredAndSortedRows = useMemo(() => {
		...
		const filtered = rows
			.filter((row) => {
				...
				if (!row.id || !row.nombre) return false;

				const matchesGlobalSearch =
					!globalTerm ||
					String(row.id).toLowerCase().includes(globalTerm) ||
					String(row.nombre).toLowerCase().includes(globalTerm);
				return (
					matchesGlobalSearch &&
					String(row.id).toLowerCase().includes(inlineFilters.id.toLowerCase()) &&
					String(row.nombre).toLowerCase().includes(inlineFilters.nombre.toLowerCase())
				);
			})

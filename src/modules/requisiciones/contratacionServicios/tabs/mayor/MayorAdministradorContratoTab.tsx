...
const empty: ServiciosPersonaContactoValues = {
	nombre: '',
	cargo: '',
	correo: '',
	telefono: '',
...
export function MayorAdministradorContratoTab({
	...
	initialValues: Partial<ServiciosPersonaContactoValues>;
	onSave: (data: ServiciosPersonaContactoValues) => void;
}) {
	...
	const {
		...
		reset,
		formState: { errors },
	} = useForm<ServiciosPersonaContactoValues>({
				...
				>
					<div className="grid grid-cols-12 gap-4">
						<div className="col-span-12 lg:col-span-3">
							<FieldRoleLabel htmlFor="cs-adm-nom">Nombre</FieldRoleLabel>
							<Input id="cs-adm-nom" {...register('nombre')} className="uppercase" />
							{errors.nombre?.message ? (
								<p className="text-[11px] mt-1 text-red-600">{errors.nombre.message}</p>
							) : null}
						</div>
						<div className="col-span-12 lg:col-span-3">
							<FieldRoleLabel htmlFor="cs-adm-car">Cargo</FieldRoleLabel>
							<Input id="cs-adm-car" {...register('cargo')} className="uppercase" />
							{errors.cargo?.message ? (
								<p className="text-[11px] mt-1 text-red-600">{errors.cargo.message}</p>
							) : null}
						</div>
						<div className="col-span-12 lg:col-span-3">
							<FieldRoleLabel htmlFor="cs-adm-mail">Correo electrónico</FieldRoleLabel>
							<Input id="cs-adm-mail" type="email" {...register('correo')} />
							{errors.correo?.message ? (
								<p className="text-[11px] mt-1 text-red-600">{errors.correo.message}</p>
							) : null}
						</div>
						<div className="col-span-12 lg:col-span-3">
							<FieldRoleLabel htmlFor="cs-adm-tel">Teléfono</FieldRoleLabel>
							<Input id="cs-adm-tel" {...register('telefono')} />
